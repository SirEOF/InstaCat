...
class ObjectParser(object):
    def __init__(self, **entries):
        for key, value in entries.iteritems():
            value2 = (ObjectParser(**value) if isinstance(value, dict) else value)
            self.__dict__[key] = value2


def sleeper(m=random.randint(100, 150)):
    s = m
    for t in range(s, -1, -1):
        minutes = t / 60
        seconds = t % 60
        sys.stdout.write('[!] Trying after %d:%2d\r' % (minutes, seconds))
        sys.stdout.flush()
        time.sleep(1.0)
...
def req(url, method, payload):
    ...
    if method == "POST":
        if payload:
            return requests.post(url, headers=header, data=payload, cookies=cookies)
        else:
            return requests.post(url, headers=header, cookies=cookies)
    elif method == "GET":
        return requests.get(url, headers=header, cookies=cookies)
